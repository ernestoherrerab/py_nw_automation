...
import network_automation.infoblox_ops.reservations.reservations as reservation_add

### VARIABLES ###
FLASK_SECRET_KEY = config("FLASK_SECRET_KEY")
INFOBLOX_UPLOAD_DIR = Path("network_automation/infoblox_ops/csv_data/")
...
@infoblox_ops.route("/reservations_csv_upload")
def reservations_csv_upload():
    """ 
    Reservations Home Data input
    """
    INFOBLOX_UPLOAD_DIR.mkdir(exist_ok=True)
    return render_template(f"{TEMPLATE_DIR}/reservations_csv_upload.html")
...
@infoblox_ops.route("/add_reservations", methods=["POST"])
def add_reservations():
    if request.method == "POST":
        if "file" in request.files:
            ...
            f.save(current_app.config["UPLOAD_FOLDER"] / secure_filename(f.filename))
            file_name = INFOBLOX_UPLOAD_DIR / f.filename
            results = reservation_add.add_reservation(file_name)
