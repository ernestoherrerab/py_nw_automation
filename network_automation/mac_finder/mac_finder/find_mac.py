...
def find_mac(hostname, host_ip, find_ip, username, password):
    ...
    while loop_break == True:
        if host_type == "cs":
            ...
            neighbor_ip, nos_type, neighbor_name = core_dev.mac_to_if(
                mac_add, vlan, core_output
            )
            if neighbor_name == None:
                result = (
                    f"The MAC is found on interface {neighbor_ip} on switch {hostname}"
                )
                print(
                    f"The MAC is found on interface {neighbor_ip} on switch {hostname}"
                )
            ...
            neighbor_ip, nos_type, neighbor_name = access_dev.mac_to_if(
                mac_add, vlan, access_output
            )
            if neighbor_name == None:
                result = (
                    f"The MAC is found on interface {neighbor_ip} on switch {hostname}"
