...
def init_nornir(username, password, task_name):
    ...
    with tqdm(total=len(managed_devs.inventory.hosts)) as progress_bar:
        results = managed_devs.run(task=task_name, progress_bar=progress_bar)
...
def get_cdp_data_task(task, progress_bar):
    """
...
def get_napalm_config(get_napalm_task, site_id):
    """Retrieve device running configuration and create a file """

    Path("configs").mkdir(exist_ok=True)
    site_path = Path("configs" / site_id).mkdir(exist_ok=True)
...
def do_audit(username, password, depth_levels=3):
    ...
    """
    print("Initializing connections to devices...")
    nr, results, _ = init_nornir(username, password, get_napalm_config)
    ### REBUILD INVENTORY FILE BASED ON THE NEIGHBOR OUTPUT ###
    print(f"Getting Audited Data...")
    for result in results.keys():
        host = str(nr.inventory.hosts[result])
        input_dict[host] = {}
        input_dict[host] = dict(nr.inventory.hosts[result])
